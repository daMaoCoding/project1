currentPageLocation = window.location.href;
	...
	*/
	...
	var queryfinInStatMatch=function(){
		...
		$.ajax({
			...
			success:function(jsonObject){
				if(jsonObject.status == 1 && jsonObject.data && jsonObject.data.sendCardArrlist.length > 0){
						var tr = '';
						 //小计
						 var counts = 0;
						 var amounts=0;
						 for(var index in jsonObject.data.sendCardArrlist){
							 var val = jsonObject.data.sendCardArrlist[index];
	                        tr += '<tr>'
				...
				//分页初始化
				showPading(jsonObject.data.sendCardPage,"fininStatMatchPage",queryfinInStatMatch);
			}
		});
	...
	jQuery(function($) {
		...
		inithancipad();
		queryfinInStatMatch();
		$("#back").attr("href","#/FinTransStat:*?type="+typetab+"&handicap="+parenthandicap
				+"&level="+parentlevel+"&parentaccount="+parentaccount+"&parentstartAndEndTime="+parentstartAndEndTime
