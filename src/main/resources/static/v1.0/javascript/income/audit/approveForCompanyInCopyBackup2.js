...
function _setButtonRight(accountId) {
    ...
    if (matchRightFlag) {
        $('#approveBankInDataTable  #toMatch #accountDiv  #P' + accountId + '   #matchRightFlagBtn').show();
    }
}
...
function _initialPageByAccounts() {
    ...
    var clickedAccountId = window.localStorage.getItem("clickedAccountLastTime");
    $('#approveBankInDataTable  toMatch').empty();
    if (accountIds && accountIds.length > 0) {
        var subLi = '';
        for (var index = 0; index < accountIds.length; index++) {
            //tab标签
            var flag = clickedAccountId ? (accounts[index] == clickedAccountId ? true : (index == 0 ? true : false)) : (index == 0 ? true : false);
            if (flag) {
                subLi += '<li class="active"   li_acccountNo="' + accounts[index] + '" li_acccountId="' + accountIds[index] + '" onclick="_liOnclick(\'' + accountIds[index] + '\',\'' + accounts[index] + '\');">';
            } else {
                subLi += '<li li_acccountNo="' + accounts[index] + '" li_acccountId="' + accountIds[index] + '"  onclick="_liOnclick(\'' + accountIds[index] + '\',\'' + accounts[index] + '\');">';
            }
            subLi +=
                '<a  data-toggle="tab" href="#account' + accountIds[index] + '">' +
                '<span class="badge badge-important" id="span' + accountIds[index] + '"></span>' +
                '<label class="accountColor" id="accountColor' + accountIds[index] + '">';
        ...
        $('#approveBankInDataTable  #toMatch   #accountsNavUL').empty().html(subLi);
        for (var i = 0, L = oldAccountStatusArray.length; i < L; i++) {
            _showAccountStatus(oldAccountIdArray[i], oldAccountStatusArray[i]);
        }
        _showLiAccount();
        $('#approveBankInDataTable  #toMatch   #accountsNavUL').show();
        $('#approveBankInDataTable  #toMatch   #accountsNavUL li.active').click();
    } else {
        $('#approveBankInDataTable  toMatch').empty().html('<div><h3 style="text-align: center;color: #6243ff">没有分配的账号</h3></div>').show();
    }
}
...
function _initalLevelCondition() {
    $('#approveBankInDataTable  #toMatch #accountDiv  .chosen-select').chosen({
    ...
    $('#approveBankInDataTable  #toMatch #accountDiv #level' + accountIdWhole).empty().html(opt);
    $('#approveBankInDataTable  #toMatch #accountDiv #level' + accountIdWhole + '_chosen').prop('style', 'width:170px;');
    $('#approveBankInDataTable  #toMatch #accountDiv #level' + accountIdWhole).trigger("chosen:updated");
    //$('#approveBankInDataTable  #toMatch  .chosen-container-single-nosearch').remove()
}
...
function _liOnclick(id, accountNo) {
    if ($('#approveBankInDataTable  #toMatch #accountDiv')) {
        $('#approveBankInDataTable  #toMatch #accountDiv').remove();
    }
    $('#approveBankInDataTable  #toMatch').append($('#accountDiv').clone());

    $('#approveBankInDataTable  #toMatch #accountDiv #level').attr('id','level'+id);
    $('#approveBankInDataTable  #toMatch #accountDiv #orderNo').attr('id','orderNo'+id);
    $('#approveBankInDataTable  #toMatch #accountDiv #timeScopeToMatch').attr('id','timeScopeToMatch'+id);
    $('#approveBankInDataTable  #toMatch #accountDiv #member').attr('id','member'+id);
    $('#approveBankInDataTable  #toMatch #accountDiv #fromMoney').attr('id','fromMoney'+id);
    $('#approveBankInDataTable  #toMatch #accountDiv #toMoney').attr('id','toMoney'+id);
    $('#approveBankInDataTable  #toMatch #accountDiv #payMan').attr('id','payMan'+id);
    $('#approveBankInDataTable  #toMatch #accountDiv #searchSys').attr('id','searchSys'+id);
    $('#approveBankInDataTable  #toMatch #accountDiv #searchBank').attr('id','searchBank'+id);
    $('#approveBankInDataTable  #toMatch #accountDiv #searchBtn').attr('id','searchBtn'+id);
    $('#approveBankInDataTable  #toMatch #accountDiv #resetValue').attr('onclick','_resetValue('+id+');');
    $('#approveBankInDataTable  #toMatch #accountDiv #resetValue').attr('id','resetValue'+id);
    $('#approveBankInDataTable  #toMatch #accountDiv #inComeRequest').attr('id','inComeRequest'+id);
    $('#approveBankInDataTable  #toMatch #accountDiv #P').attr('id','P'+id);
    $('#approveBankInDataTable  #toMatch #accountDiv #matchRightFlagBtn').attr('onclick','_beforeMatch(this);');
    $('#approveBankInDataTable  #toMatch #accountDiv #bankFlow').attr('id','bankFlow'+id);
    $('#approveBankInDataTable  #toMatch #accountDiv').show();

    var latestUpdateTime = $.session.get("latestUpdateTime" + id);
    $('#approveBankInDataTable  #toMatch #accountDiv #P' + id + '   #connectStatus').text('正常');
    $('#approveBankInDataTable  #toMatch #accountDiv #P' + id + '   #connectStatus').attr('style', 'color:white;');
    $('#approveBankInDataTable  #toMatch #accountDiv #P' + id + '   #connectStatus').parent().attr('class', 'btn btn-success btn-sm pull-left');
    if (latestUpdateTime) {
        $('#approveBankInDataTable  #toMatch #accountDiv #P' + id + ' #latestUpdateTime').html(latestUpdateTime);
    } else {
        $('#approveBankInDataTable  #toMatch #accountDiv #P' + id + ' #latestUpdateTime').html(_latestUpdateTime(new Date().getTime()));
    }
    $('#approveBankInDataTable  #toMatch #accountDiv #span' + id).text('');
    accountIdWhole = id;
    accountNoWhole = accountNo;
    _datePickerForAll($('#approveBankInDataTable  #toMatch #accountDiv #account' + accountIdWhole).find("input.date-range-picker"));
    if (accountIdWhole) {
        _changeSearch();
    }
    ...
    $('#approveBankInDataTable  #toMatch #accountDiv #accountColor' + id).prop('style', 'color:blue;');
    $('#approveBankInDataTable  #toMatch #accountDiv label[color_value=1]').each(function (i, obj) {
        if ($(obj).attr('id') != 'accountColor' + id) {
            $(obj).prop('style', 'color:#00af0e')
        }
    });
    $('#approveBankInDataTable  #toMatch #accountDiv label[color_value=3]').each(function (i, obj) {
        if ($(obj).attr('id') != 'accountColor' + id) {
            $(obj).prop('style', 'color:#D15B47')
        }
    });
    $('#approveBankInDataTable  #toMatch #accountDiv label[color_value=4]').each(function (i, obj) {
        if ($(obj).attr('id') != 'accountColor' + id) {
...
function _searchBtnClick() {
    var searchSys = $('#approveBankInDataTable  #toMatch #accountDiv  #searchSys' + accountIdWhole).prop('checked');
    var searchBank = $('#approveBankInDataTable  #toMatch #accountDiv  #searchBank' + accountIdWhole).prop('checked');
...
function _searchBtn(id, type, pageNo) {
    ...
    var level = '';
    if ($('#approveBankInDataTable  #toMatch #accountDiv  #level' + id).val() && $('#approveBankInDataTable  #toMatch #accountDiv  #level' + id).val() != '全部') {
        level = $('#approveBankInDataTable  #toMatch #accountDiv  #level' + id).val();
    }
    var payMan = '';
    if ($('#approveBankInDataTable  #toMatch #accountDiv  #payMan' + id).val()) {
        payMan = $('#approveBankInDataTable  #toMatch #accountDiv  #payMan' + id).val();
    }
    var orderNo = '';
    if ($('#approveBankInDataTable  #toMatch #accountDiv  #orderNo' + id).val()) {
        orderNo = $('#approveBankInDataTable  #toMatch #accountDiv  #orderNo' + id).val();
    }
    var member = '';
    if ($('#approveBankInDataTable  #toMatch #accountDiv  #member' + id).val()) {
        member = $('#approveBankInDataTable  #toMatch #accountDiv  #member' + id).val();
    }
    var fromMoney = '';
    if ($('#approveBankInDataTable  #toMatch #accountDiv  #fromMoney' + id).val()) {
        fromMoney = $('#approveBankInDataTable  #toMatch #accountDiv  #fromMoney' + id).val();
    }
    var toMoney = '';
    if ($('#approveBankInDataTable  #toMatch #accountDiv  #toMoney' + id).val()) {
        toMoney = $('#approveBankInDataTable  #toMatch #accountDiv  #toMoney' + id).val();
    }
    var startAndEnd = $('#approveBankInDataTable  #toMatch #accountDiv  #timeScopeToMatch' + id).val();
    var startTime = '';
    var endTime = '';
    if (startAndEnd) {
            ...
            if (res) {
                if (res.status == 1 && res.data) {
                    ...
                    if (res.data.sysOrderList && res.data.sysOrderList.length > 0) {
                        $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id).find('tbody').attr('class', "");
                        ...
                        $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id).find('tbody').empty().html(tr).append(trs);
                        $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id).find('tbody').find('td').attr('style', 'text-align:center');
                        $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id + '  #inComeRequestTd1').attr('style', 'color:white;text-align:center;');
                        $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id + '  #inComeRequestTd2').attr('style', 'color:white;text-align:center;width: 140px;');
                        ...
                        if (addRemarkRightFlag && sendMessageRightFlag && concealOrderRightFlag) {
                            if (cancelIncomeReqFlag) {
                                $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id + '  #sysOrderOperateTd').attr('style', 'width:250px;');
                            } else {
                                $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id + '  #sysOrderOperateTd').attr('style', 'width:190px;');
                            }
                        }
                        if (addRemarkRightFlag && sendMessageRightFlag && !concealOrderRightFlag) {
                            $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id + '  #sysOrderOperateTd').attr('style', 'width:160px;');
                        }
                        if (addRemarkRightFlag && !sendMessageRightFlag && concealOrderRightFlag) {
                            $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id + '  #sysOrderOperateTd').attr('style', 'width:160px;');
                        }
                        ...
                        if (!addRemarkRightFlag && !sendMessageRightFlag && concealOrderRightFlag) {
                            $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id + '  #sysOrderOperateTd').attr('style', 'width:80px;');
                        }
                        $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id).find('tbody').removeAttr('class');
                    }
                    else {
                        $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id).find('tbody').empty().html('<tr><td colspan="13"><h3 class="alert alert-success center">无记录</h3></td></tr>');
                        $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id).find('tbody').attr('class', "alert alert-success center");
                    }
                } else {
                    $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id).find('tbody').empty().html('<tr><td colspan="13"><h3 >无记录</h3></td></tr>');
                    $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id).find('tbody').attr('class', "alert alert-success center");
                }
            }
        },
    ...
    if (data) {
        $.ajax({
            ...
            success: function (res) {
                if (res) {
                    if (res.status == 1) {
                        $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id + ' #inComeRequestTd2').empty().text(parseFloat(res.data).toFixed(3));
                    }
                }
            }
        });
...
function getCompanyInMatchingCount(data, id, type) {
    $.ajax({
        ...
        success: function (res) {
            if (res) {
                if (res.status == 1) {
                    $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id + '  #inComeRequestCurrentCount').empty().text('小计：' + ((res.page.totalElements - res.page.pageSize * (res.page.pageNo)) >= 0 ? res.page.pageSize : (res.page.totalElements - res.page.pageSize * (res.page.pageNo - 1))) + '条记录');
                    $('#approveBankInDataTable  #toMatch #accountDiv  #inComeRequest' + id + '  #inComeRequestTotalCount').empty().text('合计：' + res.page.totalElements + '条记录');
                    refreshPaging(type, id, res.page, $('#approveBankInDataTable  #toMatch #accountDiv  div[id="inComeRequest' + id + '"]').find('div[id="sysPageFooter"]'));
                }
            }
        }
    });
            ...
            if (res) {
                if (res.status == 1 && res.data) {
                    ...
                    if (res.data.bankFlowList && res.data.bankFlowList.length > 0) {
                        $('#approveBankInDataTable  #toMatch #accountDiv  div[id="bankFlow' + id + '"]').find('tbody').attr('class', '');
                        ...
                        $('#approveBankInDataTable  #toMatch #accountDiv  #bankFlow' + id).find('tbody').empty().html(tr).append(trs);
                        $('#approveBankInDataTable  #toMatch #accountDiv  #bankFlow' + id).find('tbody').find('td').attr('style', 'text-align:center;');
                        $('#approveBankInDataTable  #toMatch #accountDiv  #bankFlow' + id + '  #bankFlowTd1').attr('style', 'color:white;text-align:center;width: 140px;');
                        $('#approveBankInDataTable  #toMatch #accountDiv  #bankFlow' + id + '  #bankFlowTd2').attr('style', 'color:white;text-align:center;width: 140px;');
                        _setButtonRight(id);
                        if (addRemarkRightFlag && sendMessageRightFlag && addOrderRightFlag && finalDealBankFlowFlag) {
                            $('#approveBankInDataTable  #toMatch #accountDiv  #bankFlow' + id + '  #bankFlowOperateTd').attr('style', 'width:240px;');
                        }
                        if (addRemarkRightFlag && sendMessageRightFlag && !addOrderRightFlag && finalDealBankFlowFlag) {
                            $('#approveBankInDataTable  #toMatch #accountDiv  #bankFlow' + id + '  #bankFlowOperateTd').attr('style', 'width:160px;');
                        }
                        if (addRemarkRightFlag && !sendMessageRightFlag && addOrderRightFlag && finalDealBankFlowFlag) {
                            $('#approveBankInDataTable  #toMatch #accountDiv  #bankFlow' + id + '  #bankFlowOperateTd').attr('style', 'width:160px;');
                        }
                        ...
                        if (!addRemarkRightFlag && !sendMessageRightFlag && addOrderRightFlag && !finalDealBankFlowFlag) {
                            $('#approveBankInDataTable  #toMatch #accountDiv  #bankFlow' + id + '  #bankFlowOperateTd').attr('style', 'width:60px;');
                        }
                        if (!addRemarkRightFlag && !sendMessageRightFlag && !addOrderRightFlag && finalDealBankFlowFlag) {
                            $('#approveBankInDataTable  #toMatch #accountDiv  #bankFlow' + id + '  #bankFlowOperateTd').attr('style', 'width:60px;');
                        }
                        ...
                        if (type == 1) {
                            $('#approveBankInDataTable  #toMatch #accountDiv  li[li_acccountid="' + id + '"]').find('#span' + id).text('');
                            $('#approveBankInDataTable  #toMatch #accountDiv  div[id="bankFlow' + id + '"]').find('tbody').empty().html('<tr><td colspan="10"><h3 >无流水</h3></td></tr>');
                            $('#approveBankInDataTable  #toMatch #accountDiv  div[id="bankFlow' + id + '"]').find('tbody').attr('class', '"alert alert-success center"');
                        }
                    }
                }
                else {
...
function _addIncomeRequest(id, payer, amount) {
    ...
    var accountNoShow=accountNoWhole.substring(0,4)+"***"+accountNoWhole.substring(len-4,len);
...
function _getBankFlowSum(id, data) {
    if (data) {
        $.ajax({
            ...
            success: function (res) {
                if (res && res.status == 1) {
                    $('#approveBankInDataTable  #toMatch #accountDiv  div[id="bankFlow' + id + '"]  #bankFlowTd2').text(parseFloat(res.data).toFixed(3));
                }
            }
        });
    }
        ...
        success: function (res) {
            if (res) {
                if (res.status == 1 && res.page) {
                    $('#approveBankInDataTable  #toMatch #accountDiv  #bankFlow' + id + '  #bankFlowCurrentCount').text('小计：' + ((res.page.totalElements - res.page.pageSize * (res.page.pageNo)) >= 0 ? res.page.pageSize : (res.page.totalElements - res.page.pageSize * (res.page.pageNo - 1))) + '条记录');
                    $('#approveBankInDataTable  #toMatch #accountDiv  #bankFlow' + id + '  #bankFlowTotalCount').text('合计：' + res.page.totalElements + '条记录');
                    refreshPaging(type, id, res.page, $('#approveBankInDataTable  #toMatch #accountDiv  div[id="bankFlow' + id + '"]').find('div[id="bankFlowFooter"]'));
                    if (res.page && res.page.totalElements > 0) {
                        $('#approveBankInDataTable  #toMatch #accountDiv  li[li_acccountid="' + id + '"]').find('#span' + id).text(res.page.totalElements);
                    }
                }else{
                    $('#approveBankInDataTable  #toMatch #accountDiv  li[li_acccountid="' + id + '"]').find('#span' + id).text('');
                }
...
function _checkFlowException() {
    var tradingTimeTd = $('#approveBankInDataTable  #toMatch #accountDiv  div[id="bankFlow' + accountIdWhole + '"]').find('tbody').find('td[id="tradingTimeTd"]');
...
function _awMatchingOrder() {
    ...
    if (!oid) {
        $.gritter.add({
            time: 500, class_name: '', title: '系统消息', text: '没有分配账号',
...
function _addOrderAW(oid, inAccount, userName, money, obj) {
    if(!inAccount){
        console.log("AW账号丢失:"+inAccount);
        return;
    }
    var len = inAccount.length;
    var account =inAccount.substring(0,4)+"***"+inAccount.substring(len-2,len);
