...
var loadIncomeAccountTab=function(CurPage){
	...
	$.ajax({
        ...
        success:function(jsonObject){
        	...
        	$.each(jsonObject.data,function(index,result){
        		...
        		liStr+='<li '+active+' id="accountListli'+record.id+'" onclick="changeTabAccountMatching('+record.id+');"  style="width:20%;z-index:0">\
				        	...
				        	<span class="bind_hover_card breakByWord" data-toggle="accountInfoHover'+record.id+'" data-placement="auto top" data-trigger="hover"  href="#selectedAccountsDiv">\
				        		...
				        		<label id="accountColor123" style="'+styleStr+'">'+(record.alias?record.alias:"无别名")+'|'+(record.owner?hideName(record.owner):'无')+'|'+replaceBankStr(record.bankType)+'</label>\
				        	</span>\
				        	</a>\
				        </li>';
			...
			//分页初始化
			showPading(jsonObject.page,"accountList_page",loadIncomeAccountTab,'入货账号',true);
        ...
        success: function (jsonObject) {
        	...
        	if($("#choseAccount").val()){
    			...
    			$.each(jsonObject.data,function(index,record){
    				...
    				trStr+="<td><span title='会员账号 - 付货姓名'>"+(record.memberUserName?record.memberUserName:"无")+"&nbsp;-&nbsp;"+(record.memberRealName?record.memberRealName:"无")+"</span></td>";
    			...
    			$tbody.html(trStr);
    			showPading(jsonObject.page,"incomeMatchingDiv_tablePage",loadincomeMatchingList,"入货记录");
    			if(jsonObject.data){
    				var totalRows={
    						column:12, 
        		...
        		$tbody.html(trStr);
        		showPading(null,"incomeMatchingDiv_tablePage",loadincomeMatchingList,"入货记录");
        	}
...
var loadCancelList=function(CurPage){
	...
	var accountId = $filter.find("select[name='search_EQ_id']").val();
		var data =  {
	        ...
	        'toAccount': $.trim($filter.find("[name='search_LIKE_account']").val()),
...
var loadMatchedList=function(CurPage){
	...
	var data =  {
	        ...
	        'toAccount': $.trim($filter.find("[name='search_LIKE_account']").val(),true),
	        'memberUserName': $.trim($filter.find("input[name='memberUserName']").val()),
...
var loadBankLogUnMatchList=function(CurPage){
	...
	if(!!!CurPage&&CurPage!=0) CurPage=$("#BankLogUnMatch_tablePage .Current_Page").text();
		var data =  {
			"fromAccountNO": $.trim($filter.find("input[name='search_LIKE_account']").val(),true),//from才是抓取的账号id
...
var loadFastSearchBankLogList=function(CurPage){
	...
	if(!$.trim($filter.find("input[name='search_EQ_alias']").val(),true)&&!$.trim($filter.find("input[name='toAccount']").val(),true)
		...
		showMessageForFail("请先输入至少一个查询条件（收货账号/存货账号/存货人）");
		return;
	}
	...
	$.ajax({
		...
		success: function (jsonObject) {
			...
			$.each(jsonObject.data,function(index,record){
				...
				"</a>" +
				"</td>";
				trStr+="<td><span>"+(record.toAccount?hideAccountAll(record.toAccount):'')+"</span></td>";
				trStr+="<td><span>"+(record.toAccountOwner?record.toAccountOwner:'')+"</span></td>";
				trStr+="<td><span>"+record.amount+"</span></td>";
				trStr+="<td><span>"+(record.balance?record.balance:0)+"</span></td>";
				trStr+="<td><span>"+timeStamp2yyyyMMddHHmmss(record.tradingTime)+"</span></td>";
				trStr+="<td><span>"+timeStamp2yyyyMMddHHmmss(record.createTime)+"</span></td>";
				trStr+="<td>"+getHTMLremark(record.summary,150)+"</td>";
				trStr+="<td>"+getHTMLremark(record.remark,150)+"</td>";
    ...
    $.ajax({
        ...
        success: function (jsonObject) {
			...
			if($("#choseAccount").val()){
				...
				$tbody.html(trStr);
				showPading(jsonObject.page,"BankLogMatching_tablePage",loadBankLogMatchingList,"货品流水");
				contentRight();
				if(jsonObject.data){
					var totalRows={
							column:12, 
				...
				//请空数据
				$tbody.html(trStr);
				showPading(null,"BankLogMatching_tablePage",loadBankLogMatchingList,"货品流水");
			}
        }
    });
}
	        ...
	        success: function (jsonObject) {
				...
				$.each(jsonObject.data,function(index,record){
					...
					trStr+="<td>" +
							...
							"</a>" +
						"</td>";
					trStr+="<td><span>"+replaceBankStr(record.toBankType)+"</span></td>";
...
var doAddIncomeRequest=function() {
    ...
    if (generateIncomeOrder && generateIncomeOrder == bankLogId) {
        ...
        if(!makeUpAmount) {
            $('#makeUpPrompt').text("请填写收货点数！").show(10).delay(300).hide(10);
            return false;
        }
        if(!makeUpAccount) {
            $('#makeUpPrompt').text("请填写公司收货账号！").show(10).delay(300).hide(10);
            return false;
        }
        if(!makeUpDepositType) {
            $('#makeUpPrompt').text("请选择存货类型！").show(10).delay(300).hide(10);
            return false;
        }
        if (!pfTypeSub) {
            $('#makeUpPrompt').text("请选择提货类型！").show(10).delay(1000).hide(10);
            return false;
        }
        var bankLogId = $('#bankLogIdToReBuild').val();
        var data = {
...
//正在匹配 商家，层级 收货账号change 绑定查询账号事件
