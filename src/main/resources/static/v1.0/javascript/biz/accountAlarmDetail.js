...
var authRequest = { "pageNo":0,"pageSize":10000};
...
var packSearchData4Flow=function(){
	...
	var aliasLike = request.aliasLike;
	...
	return {pageNo:0,pageSize:50,
		aliasLike:aliasLike,
		accountId:request.accountId,
		timeBtw:timeBtw,amtBtw:amtBtw,transIn0Out1:1,statusArray:statusArray,doing0OrDone1:doing0OrDone1};
};
...
var modelHtml4AccFlow_Doing = '\
	   <tr class="accFlowTr{id}">\
	       <td><input type="checkbox" name="idByBatch" value="{id}"/></td>\
		   ...
		   <td name="amount"><span style="color:#000;">{summary}</span></td>\
...
var searchByFilter = function(doingOrdone){
	...
	var params = packSearchData4Flow();
...
var doInvalidByBatch = function(){
	var array = [];
	$("input[type=checkbox][name=idByBatch]:checked").each(function(index,obj){
		array.push(obj.value);
	});
	if(array.length==0){
		showMessageForFail("请选择需要批量处理的数据");
	}
	bootbox.confirm("确定设这些流水为无效记录吗&nbsp;?&nbsp;", function(result) {
		if (result) {
			$.ajax({type: 'get',url: '/r/accountMonitor/doInvalidByBatch',data: {"ids":array.toString()},dataType: 'json',success: function (res) {
				if (res.status==1) {
...
var doExtFunds4Other = function(){
	...
	var params = {flowId:flowId,remark:remark};
	$.ajax({dataType:'json',type:"POST",url:API.r_accountMonitor_alterFlowToDisposed,data:params,success:function(jsonObject){
		if(jsonObject.status !=1){
			showMessageForFail("操作失败："+jsonObject.message);
			return;
		}
...
initTimePicker(null,null,typeCustomLatestOneDay,null);
searchByFilter();
