 ...
 * 转入按照余额确认(人工)
 */
@SuccessAnnotation(SuccessCheck.PREFIX_SUCCESS_CHECK + SuccessCheck.SUCCESS_CHECK_TYPE_OTHERS_INBYBAL_)
public class OthersInByBalSuccessCheck_ extends SuccessCheck {

	protected static final Logger logger = LoggerFactory.getLogger(OthersInByBalSuccessCheck_.class);
	...
	protected boolean deal(StringRedisTemplate template, AccountBaseInfo base, SuccessHandler handler,
		...
		if (Objects.isNull(template) || Objects.isNull(base) || Objects.isNull(param)
			...
			return false;
		return in_(handler, template, base, param.getBenchmark(), param.getBalance(), check);
	}

	protected boolean in_(SuccessHandler handler, StringRedisTemplate template, AccountBaseInfo base,
		...
		if (Objects.isNull(acc) || Objects.isNull(acc.getBalance())
				|| realBal.subtract(acc.getBalance()).compareTo(ints.getAmt()) != 0)
			return false;
		...
		logger.info(
				"SB{} SB{} [ OTHERS BANK BALANCE MANUAL INBYBAL INCOME CONFIRMED ] >> amtin: {} benchmark: {} realBal: {}  msg: {}",
