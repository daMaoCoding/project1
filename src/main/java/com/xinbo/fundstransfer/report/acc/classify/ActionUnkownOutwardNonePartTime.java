 ...
 * 盗刷流水处理方式:非兼职所为
 */
@ErrorUp(Error.ERROR_ACC + SysAccPush.ActionUnkownOutwardNonePartTime)
public class ActionUnkownOutwardNonePartTime extends Error {
	...
	public String deal(Long errorId, int target, Object data, String remark, SysUser operator, String[] others,
		...
		BizSysInvst actionFee = sysInvstSer.saveAndFlush(unkown.getFromAccount(), unkown.getId(), unkown.getAmount(),
				...
				err.getBatchNo(), null, null, SysInvstType.UnkownOutwardByNonePartTime.getType(), unkown.getSummary(),
				remark, operator.getId(), d, occurTime, d.getTime() - occurTime.getTime());
		saveErrorBankLog(unkown, operator, SysInvstType.UnkownOutwardByNonePartTime.getMessage());
		log.info(
				"INVST{} [ UNKOWNOUTWARD-NONEPARTTIME ] >>  amount:{} operator: {} remark: {} bankLogId:{}  ActionId:{}",
