...
/**
 * 人工内部转入 others[0] 对方账号流水号
 */
@ErrorUp(Error.ERROR_ACC + SysAccPush.ActionManualTransIn)
public class ActionManualTransIn extends Error {
	...
	public String deal(Long errorId, int target, Object data, String remark, SysUser operator, String[] others,
		...
		String ret = checkBankLogAmount(true, manual.getAmount(), null);// 检测：流水金额
		...
		if (Objects.equals(InBank, base.getType())) {
			BizAccount acc = accSer.getById(target);
			bs = new BigDecimal[] { acc.getBankBalance(), acc.getBalance() };
		} else {
			bs = storeHandler.setSysBal(accountingStringRedisTemplate, target, manual.getAmount(), null, false);
		}
		...
		BizSysInvst actionMaual = sysInvstSer.saveAndFlush(manual.getFromAccount(), manual.getId(), manual.getAmount(),
				...
				err.getBatchNo(), opp.getOrderId(), opp.getOrderNo(), SysInvstType.ManualTransIn.getType(), summary,
				remark, operator.getId(), d, occurTime, d.getTime() - occurTime.getTime());
		saveErrorBankLog(manual, operator, SysInvstType.ManualTransIn.getMessage());
		log.info("INVST{} [ MANUALTRANSIN ] >>  amount:{} operator: {} remark: {} bankLogId:{}  ActionId:{}",
				base.getId(), manual.getAmount(), operator.getUid(), remark, manual.getId(), actionMaual.getId());
