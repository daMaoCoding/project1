...
public class WxZfbRate3Controller extends Base3Common {
	...
	@RequestMapping(value = "/synwxzfbrate", method = RequestMethod.POST, consumes = "application/json")
	@ResponseBody
	public SimpleResponseData rate(@Valid @RequestBody ReqV3Rate requestBody, BindingResult result)
		...
		List<SysUserProfile> settingList = systemSettingService
				.findByPropertyKey(type == 0 ? UserProfileKey.REBATE_WX_SYS_RATE_SETTING.getValue()
						: UserProfileKey.REBATE_ZFB_SYS_RATE_SETTING.getValue())
				.stream().filter(p -> Objects.equals(p.getUserId(), 1)).collect(Collectors.toList());
		...
		if (Objects.isNull(profile)) {
			...
			profile.setPropertyKey(type == 0 ? UserProfileKey.REBATE_WX_SYS_RATE_SETTING.getValue()
					: UserProfileKey.REBATE_ZFB_SYS_RATE_SETTING.getValue());
			profile.setPropertyName(type == 0 ? "返利网微信返利设置" : "返利网支付宝返利设置");
		}
	...
	private boolean checkToken(ReqV3Rate arg0, String paramBody) {
		...
		items.forEach(p -> sb.append(trans2Radix(p.getAmount()).toString()).append(trans2Radix(p.getRate())).append(
				Objects.isNull(p.getUplimit()) || p.getUplimit() == 0 ? StringUtils.EMPTY : trans2Radix(p.getUplimit()))
				.append(p.getType()));
